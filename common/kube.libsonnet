...

{
  ...
  Service(name): $._Object("v1", "Service", name) {
    ...
    portName:: self.target_pod.spec.containers[0].ports[0].name,
    port:: self.target_pod.spec.containers[0].ports[0].containerPort,
    containerPort:: self.target_pod.spec.containers[0].ports[0].containerPort,
  ...
  Container(name): {
    ...
    tty: false,
    forceTTY_:: false,
    assert !self.tty || self.forceTTY_ || self.stdin : "tty=true requires stdin=true",
  },
  ...
  SecretKeyRef(secret, key): {
    assert std.objectHas(secret.data, key) || std.objectHas(secret.stringData, key): "%s not in secret.data" % [key],
    secretKeyRef: {
  ...
  Deployment(name): $._Object("apps/v1", "Deployment", name) {
    ...
    spec: {
      ...
      selector: {
        matchLabels: {
          name: deployment.spec.template.metadata.labels.name,
        }
      },
